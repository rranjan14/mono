...
const makeMutatorQueryOptions = (
  ...
  url: {
    ...
    desc: [
      ...
      ``,
      `{bold IMPORTANT:} URLs are treated as {bold regular expression patterns}, not literal strings.`,
      ``,
      `{bold Anchoring (automatic):}`,
      ...
      ``,
      `{bold Note:} Query parameters and URL fragments (#) are ignored during matching.`,
    ],
  },
  apiKey: {
    type: v.string().optional(),
